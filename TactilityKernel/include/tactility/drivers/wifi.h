 ...
 *
 * Not esp_hosted-specific: some WiFi devices are backed by a separate co-processor with its own
 * updatable firmware (e.g. esp-hosted-mcu's P4+C6/C5 pairing); most aren't. A WifiApi
 * implementation that has no such co-processor returns ERROR_NOT_SUPPORTED from
 * get_firmware_ops() below (leaving its output params untouched) - callers must check the
 * result before using any FirmwareOps function.
 */
struct FirmwareInfo {
