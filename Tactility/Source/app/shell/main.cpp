...
int main(int argc, char* argv[]) {
    ...
    // exit), which read() reports the same way any closed pipe does: 0, ending this loop.
    char c;
...
extern const ::AppManifest manifest = {
    ...
    // Only meaningful piped through a terminal app's AppStreamBinding (see Tactility/app/terminal),
    // not launched directly from an app-browsing UI.
    .flags = APP_MANIFEST_FLAG_HIDDEN,
    .stack = { .depth = 10 * 1024, .desired_memory_capability = MEMORY_CAPABILITY_INTERNAL },
};
