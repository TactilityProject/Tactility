...
// on its own event source instead of polling vterm_getchar(). Pass NULL to remove. Returns only
// once a callback already in progress has finished, so the previous context may be released after.
